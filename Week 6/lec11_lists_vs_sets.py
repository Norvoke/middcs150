...
def query_data(data, num_queries, largest):
    """
    Perform random membership queries on a collection.    
    
    Args:
        data: collection supporting in operator
        num_queries: number of queries run in test
        largest: largest integer in data                
...
def speed_data(num_queries, min_size, max_size, step_size):
    """
    ...
    for size in range(min_size, max_size, step_size):
        # Generate random list
        list_data = generate_list(size)
    
        # Generate random set
        set_data = generate_set(size)
    
        # Time the list querying
        list_elapsed = query_data(list_data, num_queries, size*10)

        # Time the list querying
        set_elapsed = query_data(set_data, num_queries, size*10)
        
        print("%d\t%f\t%f" % (size, list_elapsed, set_elapsed))
